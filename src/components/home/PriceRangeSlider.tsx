...
const PriceRangeSlider = ({ minIndex, maxIndex, onChange }: PriceRangeSliderProps) => {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-end justify-between gap-4">
        ...
        <span className="text-right text-sm font-medium tabular-nums text-white/90">
      ...
      <div ref={trackRef} className="relative h-10 select-none touch-none">
        ...
        <div
          className="absolute top-1/2 h-px -translate-y-1/2 bg-white/70"
        ...
        {(['min', 'max'] as const).map((thumb) => {
          ...
          return (
            <button
              ...
              className="absolute top-1/2 z-10 h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full border border-white/60 bg-navy-dark shadow-sm transition-transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-white/30"
              style={{ left: `${percent}%` }}
